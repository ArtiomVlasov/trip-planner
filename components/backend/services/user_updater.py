...
from services.user_resgister import replace_user_type_weights
...
def update_user_data(db: Session, user: User, processed_message: dict) -> None:
    try:
        ...
        if "preferred_types" in user_payload and user_payload["preferred_types"] is not None:
            replace_user_type_weights(db, user.id, user_payload["preferred_types"])
        else:
